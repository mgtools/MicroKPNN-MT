...
class MicroDataset(Dataset): 
	def __init__(self, data_path, metadata_path, species_dict, output): 
		...
		if os.path.exists(embedding_file_path):
			...
			# for debugging
			# self.sample_ids = sample(self.sample_ids, 1000)
			# self.species = sample(self.species, 1000)
			# self.ages = sample(self.ages, 1000)
			# self.genders = sample(self.genders, 1000)
			# self.bmis = sample(self.bmis, 1000)
			# self.bodysites = sample(self.bodysites, 1000)
			# self.diseases = sample(self.diseases, 1000)
			# self.diseases_idx = sample(self.diseases_idx, 1000)
		else:
