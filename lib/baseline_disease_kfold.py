...
def count_parameters(model):
	if isinstance(model, SVC):
		return model.support_vectors_.shape[0] * model.support_vectors_.shape[1] + model.support_vectors_.shape[0]
	elif isinstance(model, RandomForestClassifier):
		return sum(tree.tree_.node_count for tree in model.estimators_) * 3
	elif isinstance(model, XGBClassifier):
		return model.get_booster().num_features() * model.get_params()['n_estimators']
...
def train_model(X_train, y_train, checkpoint_path, model_type, seed): 
	if model_type == 'SVM': 
		...
		model = SVC(kernel='rbf', C=0.1, gamma='scale', probability=True, random_state=seed)
	...
	print(f"Number of parameters for {model_type}: {num_params}")
